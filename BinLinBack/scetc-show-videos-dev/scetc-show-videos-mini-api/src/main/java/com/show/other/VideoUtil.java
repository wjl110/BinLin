...
public class VideoUtil {
    ...
    public VideoUtil(String accessKeyId, String accessKeySecret) {
        // 设置endpoint
        try {
    ...
    public static void getVideoImgByTime(String ffmepgPath, String timeString, String videoPath, String imgPath) throws IOException {
        new File(imgPath).delete();
        new ProcessBuilder().command(ffmepgPath, "-ss", timeString, "-i", videoPath, "-frames:v", "1", imgPath).start();
    }

    public static void getWavByVideo(String ffmepgPath, String timeString, String videoPath, String imgPath) throws IOException {
        new ProcessBuilder().command(ffmepgPath, "-i", timeString, "-i", videoPath, "-frames:v", "1", imgPath).start();
    }
     ...
     * @throws Exception
     */
    public static void getSrtByWav(String wavFileLink) throws Exception {
        VideoUtil demo = new VideoUtil(accessKeyId, accessKeySecret);
        ...
        if (taskId != null) {
            ...
            if (result != null) {
                JSONArray sentences = JSONObject.parseObject(result).getJSONArray("Sentences");
            }
        }
    }
